...
(function() {
  ...
  window.Milk = (function() {
    ...
    Milk.prototype.exportObject = function(object3D, bounds) {
      this.object3D = object3D;
      if (bounds) {
        this.bounds = bounds;
      }
      return this.componentDispatch('exportObject', [object3D, bounds]);
    };
    ...
    Milk.prototype.fire = function(eventName, data) {
      var c, callback, _i, _len, _ref;
      c = (_ref = this._observers) != null ? _ref[eventName] : void 0;
      if (c) {
  ...
  Milk.Game = (function(_super) {
    ...
    Game.prototype.debug = function(key) {
    ...
    Game.prototype.ready = function() {
      ...
      setTimeout((function() {
        return document.getElementById('chat-log').innerHTML = '';
      }), 1500);
      this.stage();
      return this.start();
    };
  ...
  */
  ...
  Milk.Actor = (function(_super) {
    ...
    Actor.prototype.unstage = function() {
  ...
  Milk.Alien = (function(_super) {
    ...
    function Alien() {
      ...
      this.character.onLoadComplete = function() {
        var bounds, box, scale;
        _this.character.meshBody.rotation.y = 1.5;
        box = _this.character.meshBody.geometry.boundingBox.max;
        scale = _this.character.meshBody.scale;
        bounds = new THREE.Vector3().multiply(box, scale);
        _this.exportObject(_this.character.root, bounds);
        return _this.ready();
      };
  ...
  Milk.Spaceman = (function(_super) {
    ...
    function Spaceman() {
      ...
      this.sprite = new Milk.Sprite("public/robot.png", function() {
        _this.exportObject(_this.sprite.object3D, _this.sprite.bounds);
        return _this.ready();
      });
    }
  ...
  */
  ...
  Milk.MoonLevel = (function(_super) {
    ...
    function MoonLevel() {
      ...
      this.receiveChangePlayerActor = __bind(this.receiveChangePlayerActor, this);
      ...
      game.client.observe('receiveChangePlayerActor', this.receiveChangePlayerActor);
    ...
    MoonLevel.prototype.addPlayer = function(data) {
      var components, player, ready,
        _this = this;
      components = [Milk.Movable, Milk.Animation, Milk.OverheadText];
      if (data.controllable) {
        components.push(Milk.Controllable);
      }
      if (typeof data.actorClass === 'string') {
        data.actorClass = Milk[data.actorClass];
      }
      player = (function(func, args, ctor) {
        ctor.prototype = func.prototype;
        var child = new ctor, result = func.apply(child, args), t = typeof result;
        return t == "object" || t == "function" ? result || child : child;
      })(data.actorClass || Milk.Spaceman, components, function(){});
      this.players[data.id] = player;
      ready = function() {
        player.stage();
        return player.receiveNetworkUpdate(data);
      };
      if (player.isReady()) {
        ready();
      } else {
        player.observe('ready', ready);
      }
      return player;
    };
    ...
    MoonLevel.prototype.removePlayer = function(data) {
      var player;
      player = this.players[data.id] || data;
      player.unstage();
      this.players[data.id] = null;
      return delete this.players[data.id];
    };

    MoonLevel.prototype.replacePlayerActor = function(player, actorClass, options) {
      if (options == null) {
        options = {};
      }
      ...
      return this.addPlayer(options);
    };

    MoonLevel.prototype.receiveChangePlayerActor = function(data) {
      var player;
      if (!(player = this.players[data.id])) {
        return;
      }
      return this.replacePlayerActor(player, data.actorClass, data);
    };
  ...
  Milk.NetworkChat = (function(_super) {
    ...
    NetworkChat.prototype.sendMessage = function() {
      var data, message, params, script;
  ...
  Milk.NetworkClient = (function(_super) {
    ...
    function NetworkClient() {
      ...
      now.receiveChangePlayerActor = function(data) {
        if (data.id === _this.id()) {
          return;
        }
        return _this.fire('receiveChangePlayerActor', data);
      };
    }
    ...
    NetworkClient.prototype.stage = function() {
      var _this = this;
    ...
    NetworkClient.prototype.unstage = function() {
  ...
  */
  ...
  Milk.ChangePlayer = (function(_super) {

    __extends(ChangePlayer, _super);

    function ChangePlayer(className) {
      var player,
        _this = this;
      ...
      player = game.level.replacePlayerActor(game.level.player, className, {
        controllable: true,
        id: 'player'
      });
      player.observe('ready', function() {
        return game.level.player = player;
      });
      now.changePlayerActor({
  ...
  Milk.Sprite = (function(_super) {
    ...
    function Sprite(filename, callback) {
      ...
      this.texture = THREE.ImageUtils.loadTexture(this.filename, null, function() {
        ...
        mesh.scale.x = _this.texture.image.width * SCALE_FACTOR;
        mesh.scale.y = _this.texture.image.height * SCALE_FACTOR;
        _this.exportObject(mesh, mesh.scale);
        if (callback != null) {
          callback.call(_this);
        }
        return _this.ready();
  ...
  Milk.OverheadText = (function(_super) {
    ...
    OverheadText.prototype.update = function() {
      ...
      position.y += ((_ref = this.bounds) != null ? _ref.y : void 0) || 1.5;
      this.text.object3D.lookAt(game.level.camera.position);
      return this.text.object3D.translateX(-(this.text.bounds.x * 0.5));
    };
